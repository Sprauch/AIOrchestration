"""Tech Lead Agent — reviews proposals, design feedback, and produces task assignments."""
...
class TechLeadAgent(AgentProcess):
...
class TechLeadDeliberatingAgent(DeliberatingAgent, TechLeadAgent):
    def default_channels(self) -> dict:
        return TechLeadAgent.default_channels(self)

    def format_prompt(self, envelope: Envelope) -> str:
        return TechLeadAgent.format_prompt(self, envelope)

    def parse_response(self, raw: str, source_envelope: Envelope) -> list[Envelope]:
        return TechLeadAgent.parse_response(self, raw, source_envelope)
