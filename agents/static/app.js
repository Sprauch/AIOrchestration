...
const _stageHelp={
  ...
  'Proposed':'PM submitted a proposal, waiting for architect review',
...
function initials(r) { return {pm:'PM',architect:'AR',developer:'DV',reviewer:'RV',orchestrator:'OR'}[r]||'??'; }
function roleTitle(r) { return {pm:'Product Manager',architect:'Architect',developer:'Developer',reviewer:'Reviewer'}[r]||r; }
...
async function refreshOverview() {
  try {
    ...
    if(as){
      ...
      as.innerHTML=['pm','architect','developer','reviewer'].map(r=>{
        const agents=byR[r]||[];
...
async function refreshRedisInspector() {
  try {
    ...
    if (aw) {
      ...
      const labels = {'orchestrator:active:proposals':'Proposals','orchestrator:active:tasks':'Tasks','orchestrator:active:reviews':'Reviews'};
      let awH = '';
...
// ── Pipeline ──
const PIPELINE_ROLES=['pm','architect','developer','reviewer'],PIPELINE_CONNECTIONS=[['pm','architect'],['architect','developer'],['developer','reviewer'],['reviewer','developer']],DELIBERATION_ROLES=['pm','architect'];
const PIPELINE_QUEUE_MAP={pm_architect:'proposals',architect_developer:'tasks',developer_reviewer:'reviews'};
const PIPELINE_STAGE_QUEUE={architect:'proposals',developer:'tasks',reviewer:'reviews'};
const PIPELINE_QUEUE_LABELS={proposals:'proposal',tasks:'task',reviews:'review'};
...
function _renderPipeline(cId,sId,agents,challengers,opts){
  ...
  const _nodeDir={pm:'above',architect:'below',developer:'above',reviewer:'below'};
...
const ROLE_COLORS={pm:'var(--blue)',architect:'var(--orange)',developer:'var(--green)',reviewer:'var(--purple)'};
...
function renderTokenUsage(mt){
  ...
  ['pm','architect','developer','reviewer'].forEach(r=>{
